...
public class IndexingRealtimeSentenceMatcher implements RealtimeSentenceMatcher {
    ...
    private static class CaptureMatch implements Match {
        @Getter private int start;
        @Getter private int end;
        @Getter private CaptureGroups captureGroups;
    ...
    // A Matches implementation that handles capture groups.  It delegates all operations to
    // a VectorMatches except for the actual match retrieval, which returns a CaptureMatch.
    private static class CaptureVectorMatches implements Matches {
        private interface exclude { Match get(int pos); }
        @Delegate(excludes = exclude.class)
        private VectorMatches matches;
        private CapturingVector vector;

        public CaptureVectorMatches(CapturingVector v) {
            this.vector = v;
            matches = new VectorMatches(v);
        }
